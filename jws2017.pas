...
begin
 WHILE Length(Account) < 14
  ...
  MessageBeep(0);
//  Showmessage('¦L+ ьхэ№Їх 6-Ґш чэръ¦т!');
  Exit;
  end;
...
begin
  ...
  begin
    case M of
        1 : GetMonthStr :='січень';
        2 : GetMonthStr :='лютий';
        3 : GetMonthStr :='березень';
        4 : GetMonthStr :='квітень';
...
begin
  I_Tmp := DayOfWeek(D);
  Case I_Tmp of
    1: GetDayOfWeek := 'неділя';
    2: GetDayOfWeek := 'понеділок';
    3: GetDayOfWeek := 'вівторок';
...
begin
  CCCTmp := CCCC;
    ...
    if CCCTmp = 'і' then CCCTmp := 'І';
    if CCCTmp = 'ї' then CCCTmp := 'Ї';
    if CCCTmp = 'є' then CCCTmp := 'Є';
    if CCCTmp = 'ґ' then CCCTmp := 'Ґ';
...
begin
  ...
  begin
    S_Tmp3 := '';
    if S_Tmp[1] ='1' then S_Tmp3 := 'один мільярд';
    if S_Tmp[1] ='2' then S_Tmp3 := 'два мільярди';
    S_Tmp2 := S_Tmp3+' '+S_Tmp2;
  end;
...
begin
  ...
  Str(Kop :2, S_Tmp);
  if S_Tmp[1]=' ' then S_Tmp[1] := '0';
  if Grivna<0 then S_Result := 'мінус '+ S_Result;
  S_Result := S_Result +' '+S_Tmp;
  Case Kop of
    1,21,31,41,51,61,71,81,91: S_Tmp := 'копійка';
    2..4,22..24,32..34,42..44,52..54,62..64,72..74,82..84,92..94: S_Tmp := 'копійки';
    else S_Tmp := 'копійок'
  end;
